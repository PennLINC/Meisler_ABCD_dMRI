...
# ============================================================
# CALCULATE AGE EFFECTS BY VENDOR (TASK MODE)
# ============================================================
# Purpose:
#   Run one (metric x qc_metric) task. For each vendor (GE, Philips, Siemens),
#   fit age effects on both raw and harmonized data and write one RDS per vendor.
#   Full: value ~ s(age, k=4) + sex + s(quality_covariate, k=4) or + sex if no_quality. No batch.
#   Reduced: drop s(age) to get age effect.
# ============================================================
...
parse_args <- function() {
  ...
  if (!nzchar(metric) || !nzchar(qc_metric)) {
    stop("Usage: calculate_age_effects_vendorwise.R --metric <metric> --qc_metric <qc_metric|no_quality>")
  }
...
log_info("Running vendorwise age effects for metric:", metric, "| qc_metric:", qc_metric, "| all vendors")
...
out_dir <- fs::path(PROJECT_ROOT, "data", "age_effects", "age_effects_outputs_vendorwise")
...
out_files <- fs::path(out_dir, paste0(metric, "__", qc_safe, "__", vendors, "_age_effects_by_vendor.rds"))
if (all(file_exists(out_files))) {
  log_info("All vendor outputs already exist, skipping.")
  quit(status = 0)
}

run_for_source <- function(df, source_label, metric, qc_metric, is_no_quality, vendor, bundle_cols = NULL) {
  ...
  if (length(bundle_cols) == 0) {
    log_info("No ", source_label, " bundle columns for ", metric, " - skipping.")
    return(tibble())
  }
  if (!is_no_quality && !qc_metric %in% names(df)) {
    log_info("QC metric ", qc_metric, " not in ", source_label, " - skipping.")
    return(tibble())
  }
  ...
  if (nrow(df_vendor) < 50) {
    log_info("Too few rows for vendor ", vendor, " in ", source_label, " - skipping.")
    return(tibble())
  }

  log_info("Running age effects for source:", source_label, "| vendor:", vendor, "| qc_metric:", qc_metric, "| bundles:", length(bundle_cols))
  ...
  for (bundle_col in sort(bundle_cols)) {
    ...
    if (is.null(full_mod) || is.null(red_age_mod)) next
...
for (v in vendors) {
  out_file <- fs::path(out_dir, paste0(metric, "__", qc_safe, "__", v, "_age_effects_by_vendor.rds"))
  if (file_exists(out_file)) {
    log_info("Output already exists, skipping vendor:", v)
    next
  }
  res_harm <- run_for_source(df_harm, "harmonized", metric, qc_metric, is_no_quality, vendor = v, bundle_cols = harm_bundle_cols)
  res_raw <- run_for_source(df_raw, "raw", metric, qc_metric, is_no_quality, vendor = v, bundle_cols = raw_bundle_cols)
  res_all <- bind_rows(res_raw, res_harm)
  if (nrow(res_all) > 0) {
    saveRDS(res_all, out_file)
    log_info("Saved vendorwise age effects for metric:", metric, "| qc_metric:", qc_metric, "| vendor:", v)
  } else {
    log_info("No valid vendorwise age effects for metric:", metric, "| qc_metric:", qc_metric, "| vendor:", v)
  }
}
