...
# ============================================================
# CALCULATE QUALITY EFFECTS BY VENDOR (HARMONIZED ONLY)
# ============================================================
# Purpose:
#   Run one (metric x qc_metric) task. For each vendor (GE, Philips, Siemens), fit quality
#   effects on harmonized data and write one RDS per vendor.
#   Full: value ~ s(age,k=4) + sex + s(qc_var,k=4) + batch. Reduced for quality: drop s(qc_var).
#   image_quality_metrics only (no no_quality).
# ============================================================
...
parse_args <- function() {
  ...
  if (!nzchar(metric) || !nzchar(qc_metric)) {
    stop("Usage: calculate_quality_effects_vendorwise.R --metric <metric> --qc_metric <qc_metric>")
  }
...
log_info("Running vendorwise quality effects for metric:", metric, "| qc_metric:", qc_metric)

harmonized_file <- fs::path(PROJECT_ROOT, "data", "harmonized_data", "merged_data_meisler_analyses_harmonized.parquet")
out_dir <- fs::path(PROJECT_ROOT, "data", "quality_effects", "quality_effects_outputs_vendorwise")
...
out_files <- fs::path(out_dir, paste0(metric, "__", qc_safe, "__", vendors, "_quality_effects_by_vendor.rds"))
...
run_for_vendor <- function(df_all, vendor, metric, qc_metric, bundle_cols) {
  ...
  for (bundle_col in sort(bundle_cols)) {
    ...
    filter(!is.na(value), !is.na(age), !is.na(sex), !is.na(qc_var), !is.na(batch_device_software))
  ...
  if (is.null(full_mod) || is.null(red_qc_mod)) next
...
for (v in vendors) {
  out_file <- fs::path(out_dir, paste0(metric, "__", qc_safe, "__", v, "_quality_effects_by_vendor.rds"))
  if (file_exists(out_file)) {
    log_info("Output already exists, skipping vendor:", v)
    next
  }
  res <- run_for_vendor(df, v, metric, qc_metric, harm_bundle_cols)
  if (nrow(res) > 0) {
    saveRDS(res, out_file)
    log_info("Saved quality effects for metric:", metric, "| qc_metric:", qc_metric, "| vendor:", v, "| file:", out_file)
  } else {
