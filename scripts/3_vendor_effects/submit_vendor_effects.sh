...
#SBATCH --job-name=vendor_effects
#SBATCH --output=logs/vendor_effects_%A_%a.out
#SBATCH --error=logs/vendor_effects_%A_%a.err
...
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/calculate_vendor_effects.R" ]; then
...
"$RSCRIPT_BIN" "./calculate_vendor_effects.R" --metric "$METRIC_NAME" --qc_metric "$QC_NAME"
