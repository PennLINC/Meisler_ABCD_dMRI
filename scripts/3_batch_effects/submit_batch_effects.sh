...
#   sbatch --array=253-294,421-462,547-588,1177-1218,1261-1302 submit_batch_effects.sh
#
...
#SBATCH --output=logs/batch_effects_%A_%a.out
#SBATCH --error=logs/batch_effects_%A_%a.err
...
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/calculate_batch_effects.R" ]; then
...
echo "TASK_ID=$TASK_ID TOTAL=$TOTAL METRIC_IDX=$METRIC_IDX QC_IDX=$QC_IDX"
echo "metric=$METRIC_NAME qc_metric=$QC_NAME"

"$RSCRIPT_BIN" "./calculate_batch_effects.R" --metric "$METRIC_NAME" --qc_metric "$QC_NAME"
